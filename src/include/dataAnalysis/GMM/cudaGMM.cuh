...
public:
  ...
  __host__ int initGMM() {
    ...
    while (step < paramHostPtr->maxIteration) {
      ...
      if (std::isnan(logLikelihood) ||
          fabs(logLikelihood - logLikelihoodOld) < paramHostPtr->threshold ||
  ...
  __host__ int outputGMM(int convergeStep, std::string outputPath) {
    ...
    cudaErrChk(cudaMemcpyAsync(weight, weightCUDA, sizeof(T) * numComponents,
                               cudaMemcpyDefault, GMMStream));
    cudaErrChk(cudaMemcpyAsync(mean, meanCUDA,
                               sizeof(T) * numComponents * dataDim,
                               cudaMemcpyDefault, GMMStream));
    cudaErrChk(cudaMemcpyAsync(coVariance, coVarianceCUDA,
                               sizeof(T) * numComponents * dataDim * dataDim,
                               cudaMemcpyDefault, GMMStream));
    cudaErrChk(cudaMemcpyAsync(coVarianceDecomposed, coVarianceDecomposedCUDA,
                               sizeof(T) * numComponents * dataDim * dataDim,
                               cudaMemcpyDefault, GMMStream));
    cudaErrChk(cudaMemcpyAsync(normalizer, normalizerCUDA,
                               sizeof(T) * numComponents, cudaMemcpyDefault,
                               GMMStream));
    ...
    { // output the GMM to json
      ...
      if (std::isnan(logLikelihood))
        std::cerr << "[!]LogLikelihood " << outputPath << " is NaN!"
                  << " GMM step: " << convergeStep << std::endl;
...
private:
  ...
  void normalizeDataBack() {
    ...
    cudaGMMWeightKernel::normalizeMeanAndCovBack<T, dataDim>
        <<<1, paramHostPtr->numComponents, 0, GMMStream>>>(
            meanCUDA, coVarianceCUDA, paramHostPtr->numComponents,
            dataHostPtr->maxVelocityArray);
  }
  ...
  T sumLogLikelihood() { // its sync now, but can be async with the M step
    ...
    cudaReduction::reduceSumPreProcess<T, blockSize,
                                       cudaReduction::PreProcessType::none,
                                       void, U, true> // weighted sum
