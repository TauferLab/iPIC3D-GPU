...
using velocitySoA =
    particleArraySoA::particleArraySoACUDA<cudaParticleType, 0, 3>;
using namespace std;
...
public:
  dataAnalysisPipelineImpl(c_Solver &KCode) {
    ...
    if constexpr (VELOCITY_HISTOGRAM_ENABLE) { // velocity histogram
      ...
      velocityHistogram = new velocityHistogram::velocityHistogram(
  ...
  void writeGMMResults() {
    ...
    for (auto &speciesResArray : gmmResults) {
      ...
      for (auto &plane : speciesResArray) {
        string planePath = GMMSubDomainOutputPath + "species" +
                           std::to_string(i) + "_" + uvw[j] + ".json";
        GMMResult<GMMType, GMM_DATA_DIM>::outputResultArray(plane, planePath,
                                                            uvw[j]);
        j++;
      }
      i++;
...
int dataAnalysisPipelineImpl::GMMAnalysisSpecies(const int cycle,
  ...
  auto GMMLambda = [=](int i) mutable {
    ...
    // it is assumed that GMM_DATA_DIM == 2 and thta the velocity range is
  ...
  for (int i = 0; i < ns; i++) {
    if constexpr (VELOCITY_HISTOGRAM_ENABLE) {
      ...
      auto histogramSpeciesOutputPath =
          HistogramSubDomainOutputPath + "species" + std::to_string(i) + "/";
      velocityHistogram->init(velocitySoACUDA, cycle, i, streams[i]);
      if constexpr (HISTOGRAM_OUTPUT)
        velocityHistogram->writeToFile(histogramSpeciesOutputPath,
                                       streams[i]); // TODO
      else
...
void dataAnalysisPipelineImpl::outputGMMADIOS2() {
  CALI_CXX_MARK_SCOPE("adios_output_gmm");
  if constexpr (!GMM_OUTPUT)
    return;
  ...
  for (int i = 0; i < ns; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      varReal[i][j][0] = ioGMM.DefineVariable<GMMType>(
          varName + "_weight", shape, {0}, shape, false);
      varReal[i][j][1] = ioGMM.DefineVariable<GMMType>(varName + "_mean", shape,
                                                       {0}, shape, false);
      varReal[i][j][2] = ioGMM.DefineVariable<GMMType>(
          varName + "_coVariance", shape, {0}, shape, false);

      // loglikelihood
      std::string logLikelihoodVarName =
          "species" + std::to_string(i) + "_" + uvw[j] + "_logLikelihood";
      varReal[i][j][3] = ioGMM.DefineVariable<GMMType>(logLikelihoodVarName);

      // convergence step
      std::string convergeStepVarName =
          "species" + std::to_string(i) + "_" + uvw[j] + "_convergeStep";
      varInt[i][j][0] = ioGMM.DefineVariable<int>(convergeStepVarName);

      // simulation step
      std::string simuStepVarName =
          "species" + std::to_string(i) + "_" + uvw[j] + "_simulationStep";
      varInt[i][j][1] = ioGMM.DefineVariable<int>(simuStepVarName);

      // number of components
      std::string numComponentsVarName =
          "species" + std::to_string(i) + "_" + uvw[j] + "_numComponents";
  ...
  for (int i = 0; i < cycleCount; i++) {
    ...
    for (int j = 0; j < ns; j++) {
      for (int k = 0; k < 3; k++) {
        ...
        engineGMM.Put<GMMType>(varReal[j][k][0],
                               gmmResults[j][k][i].weight.get());
        engineGMM.Put<GMMType>(varReal[j][k][1],
                               gmmResults[j][k][i].mean.get());
        engineGMM.Put<GMMType>(varReal[j][k][2],
                               gmmResults[j][k][i].coVariance.get());
      }
    }
    engineGMM.EndStep();
  ...
  auto writeVctMapping = [&](const std::string &filePath) {
    ...
    if (vctMapping.is_open()) {
      ...
      throw std::runtime_error(
          "[!]Error: Can not create VCT mapping for velocity GMM species");
    }
  };

  if constexpr (VELOCITY_HISTOGRAM_ENABLE && HISTOGRAM_OUTPUT) {
    auto histogramSubDomainOutputPath =
        HISTOGRAM_OUTPUT_DIR + "subDomain" + std::to_string(myrank) + "/";
